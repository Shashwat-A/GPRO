...
function play3() {
  if(gameState === "play3") {
    queChBn = createSprite(1100, 730, 50, 30);

    if(ques === 1) {
      fill("white");
      textSize(40);
      text("Question: 1", width/2 - 100, 50);
      que1 = new Question("What is the minimum distance we should maintain ?", question1);
      que1.display(que1.op3);  

      if(mousePressedOver(queChBn)) {
        if(flag === true) {
          console.log("hello");
          que1.op1.destroy();
          console.log("hello2");
    ...
    if(ques === 2) {
      ...
      que1.op3.destroy();
      que1.op4.destroy();*/
      //to make a button to change the question
